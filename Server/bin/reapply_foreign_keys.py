...
parser.add_argument('--nonpartman', action="store_true", help="If the partition set you are running this on is not managed by sys_partman, set this flag. Otherwise internal sys_partman functions are used and this script may not work. When this is set the order that the tables are rekeyed is alphabetical instead of logical.")
...
def apply_foreign_keys(conn, child_tables, partman_schema):
    ...
    for c in child_tables:
        sql = """SELECT sys_get_constraintdef(con.oid) AS constraint_def
