...
parser = argparse.ArgumentParser(description="This script will dump out and then drop all tables contained in the designated schema using sys_dump.  Each table will be in its own separate file along with a SHA-512 hash of the dump file. Tables are not dropped from the database if sys_dump does not return successfully. All dump_* option defaults are the same as they would be for sys_dump if they are not given.", epilog="NOTE: The connection options for psyocpg and sys_dump were separated out due to distinct differences in their requirements depending on your database connection configuration.")
parser.add_argument('-n','--schema', help="(Required) The schema that contains the tables that will be dumped.")
parser.add_argument('-c','--connection', default="host=", help="""Connection string for use by psycopg. Role used must be able to select sys_catalog.sys_tables in the relevant database and drop all tables in the given schema.  Defaults to "host=" (local socket). Note this is distinct from the parameters sent to sys_dump.""")
...
parser.add_argument('-d','--dump_database', help="Used for sys_dump, same as its --dbname (-d) option or final database name parameter.")
parser.add_argument('--dump_host', help="Used for sys_dump, same as its --host (-h) option.")
parser.add_argument('--dump_username', help="Used for sys_dump, same as its --username (-U) option.")
parser.add_argument('--dump_port', help="Used for sys_dump, same as its --port (-p) option.")
parser.add_argument('--sys_dump_path', help="Path to sys_dump binary location. Must set if not in current PATH.")
parser.add_argument('--Fp', action="store_true", help="Dump using sys_dump plain text format. Default is binary custom (-Fc).")
...
parser.add_argument('--version', action="store_true", help="Print out the minimum version of sys_partman this script is meant to work with. The version of sys_partman installed may be greater than this.")
...
def get_tables():
    conn = psycopg2.connect(args.connection)
    cur = conn.cursor()
    sql = "SELECT tablename FROM sys_catalog.sys_tables WHERE schemaname = %s";
...
def perform_dump(result):
    ...
    processcmd = []
    if args.sys_dump_path != None:
        processcmd.append(args.sys_dump_path)
    else:
        processcmd.append("sys_dump")
    ...
    try:
        ...
        print("Error in sys_dump command: " + str(e.cmd))
        sys.exit(2)
