...
echo "check the db to see if it is alive" 2>&1
...
#1. check db if down
    ...
    then
        ...
        then
            errorhandle "exit" "`date +'%Y-%m-%d %H:%M:%S'` CHECK The process was started before checkpoint , but no pid was foud in system,, which db may have been turned off! exit"
        fi
    else
         errorhandle "exit" "`date +'%Y-%m-%d %H:%M:%S'` CHECK The process was started before checkpoint , but no pid file was foud, which db may have been turned off! exit "
    fi
    ...
    then
        ...
        echo before checkpoint query detail[$result_of_ksql] , try again! 2>&1
...
if [ $db_alive -eq 0 ]
then
    echo "kingbase is down,after retry ${HEALTH_CHECK_MAX_RETRIES} times ,cannot do checkpoint, will exit" 2>&1
    exit 66;
fi

#execute checkpoint
echo "execute checkpoint" 2>&1
ksql "port=$KB_PORT user=$KB_USER  password=$KB_REAL_PASS dbname=$KB_DATANAME connect_timeout=10" -c 'checkpoint;' 2>&1
result_of_checkpoint=$?
if [ $result_of_checkpoint -ne 0 ]
then
    echo "checkpoint execute failed ,will exit " 2>&1
    exit 66
fi

echo "check the db to see if it is alive after execute checkpoint " 2>&1
db_alive=0
for((i=1;i<=$HEALTH_CHECK_MAX_RETRIES;i++))
do
#1. check db if down
    ...
    then
        ...
        if [ "$result_of_kingbase_exist" -eq 0 ]
        then
            errorhandle "exit" "`date +'%Y-%m-%d %H:%M:%S'` CHECK The process was started after checkpoint , but no pid was foud in system,, which db may have been turned off! exit"
        fi
    else
        errorhandle "exit" "`date +'%Y-%m-%d %H:%M:%S'` CHECK The process was started after checkpoint , but no pid file was foud, which db may have been turned off! exit "
    fi
    echo "ksql \"port=$KB_PORT user=$KB_USER  dbname=$KB_DATANAME connect_timeout=10\"  -c \"select 33333;\" "
    result_of_ksql=`ksql "port=$KB_PORT user=$KB_USER  password=$KB_REAL_PASS dbname=$KB_DATANAME connect_timeout=10" -c "select 33333;"`
    rightnum=`echo $result_of_ksql | grep 33333 |wc -l; echo ";" ${PIPESTATUS[*]}`
    ...
    then
        ...
        echo "kingbase is down,retry check db is if alive,retry times:[${i}/${HEALTH_CHECK_MAX_RETRIES}]"  2>&1
        echo after checkpoint query detail[$result_of_ksql] , try again! 2>&1
        sleep $HEALTH_CHECK_RETRY_DELAY
    fi
done
if [ $db_alive -eq 0 ]
