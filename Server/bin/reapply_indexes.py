...
def create_index(conn, partman_schema, child_schemaname, child_tablename, child_index_list, parent, parent_index_list):
    ...
    for i in parent_index_list:
        ...
        if i[1] == True and args.primary:
            ...
            name_counter = 1
            while True:
                sql = "SELECT count(*) FROM sys_class c JOIN sys_namespace n ON c.relnamespace = n.oid WHERE n.nspname = %s AND c.relname = %s"
                cur = conn.cursor()
...
def get_child_index_list(conn, child_schemaname, child_tablename):
    ...
    sql = """
            ...
            SELECT sys_get_indexdef(indexrelid) AS statement
            , i.indisprimary
            ...
            JOIN sys_catalog.sys_namespace n ON c.relnamespace = n.oid
            LEFT JOIN sys_catalog.sys_constraint t ON c.oid = t.conindid
...
def get_parent_index_list(conn, parent):
    ...
    sql = """
            ...
            SELECT
            sys_get_indexdef(indexrelid) AS statement
            , i.indisprimary
