...
def get_partman_schema(conn):
    cur = conn.cursor()
    query = "SELECT nspname FROM sys_catalog.sys_namespace n, sys_catalog.sys_extension e WHERE e.extname = 'sys_partman' AND e.extnamespace = n.oid"
    cur.execute(query)
...
def get_quoted_parent_table(conn):
    cur = conn.cursor()
    query = "SELECT schemaname, tablename FROM sys_catalog.sys_tables WHERE schemaname||'.'||tablename = %s"
...
def reset_autovacuum(partman_schema, quoted_parent_table):
    ...
    # Do not use show_partitions() so that this can work on non-sys_partman partition sets
