...
function del_backup_file()
{
    ...
    if [ $keep_day -eq 0 -a $num_keep -eq 0 ]
    then
        if [ $archive_clean_flag -eq 1 ]
        then
            echo "`date` WARNING: keep_day and physical_keep_backup_file_num can not set 0 at same time."
            echo "`date` WARNING: keep_day and physical_keep_backup_file_num can not set 0 at same time." >> $backup_path/backup.log
            echo "`date` WARNING: at least keep one backup file, set physical_keep_backup_file_num=1"
            echo "`date` WARNING: at least keep one backup file, set physical_keep_backup_file_num=1" >> $backup_path/backup.log
        else
            echo "`date` WARNING: keep_day and logical_keep_backup_file_num can not set 0 at same time."
            echo "`date` WARNING: keep_day and logical_keep_backup_file_num can not set 0 at same time." >> $backup_path/backup.log
            echo "`date` WARNING: at least keep one backup file, set logical_keep_backup_file_num=1"
            echo "`date` WARNING: at least keep one backup file, set logical_keep_backup_file_num=1" >> $backup_path/backup.log
        fi
        num_keep=1
    fi
    ...
    then
        ...
        do
            if [ ! -f "${item}" ];then
                ...
                then
                    file_name=${item##*/}
                    start_wal_file=${file_name:31:24}
                    echo "`date` rm -fr --interactive=never $item"
    ...
    then
        ...
        then
            ...
            echo "`date` ${kb_path}/sys_archivecleanup $archive_path $backup_history_file -d"
            ...
            find $archive_path -mtime +3 -name "*.backup" -exec rm -rf {} \; >> $backup_path/backup.log 2>&1
...
function physical_backup()
{
    ...
    then
        ...
        tar zcvf physical_backup_${date}_${start_wal_file}.tar.gz ${date}_data 1>/dev/null 2>&1
        if [ $? -eq 0 ]
        then
            rm -fr --interactive=never $backup_path/physical/${date}_data
            rm -f --interactive=never $backup_path/physical/${date}_data.ing
            echo "`date` physical backup and tar physical_backup_${date}_${start_wal_file}.tar.gz done."
        else
