...
parser.add_argument('--all', action="store_true", help="Run against all tables managed by sys_partman. Either this option or -p is required. Overrides -p if both are set.")
...
def get_partition_list(conn, parent_table, vacuum_freeze_min_age):
    ...
    # Do not use show_partitions() so this can work on non-sys_partman tables
    sql = """
        WITH parent_info AS (
            SELECT c1.oid FROM sys_catalog.sys_class c1
    ...
    for c in child_list:
        sql = """SELECT age(relfrozenxid) FROM sys_catalog.sys_class c JOIN sys_catalog.sys_namespace n ON c.relnamespace = n.oid
                WHERE n.nspname = %s
                AND c.relname = %s"""
        if args.debug:
    ...
    sql = "SELECT schemaname, tablename FROM sys_catalog.sys_tables WHERE schemaname||'.'||tablename = %s"
    if args.debug:
        print(cur.mogrify(sql, [parent_table]))
    cur.execute(sql, [parent_table])
    ...
    if args.interval != None:
        ...
        if args.type == "id":
            ...
            sql = "SELECT CURRENT_TIMESTAMP - %s::interval"  # Use CURRENT_TIMESTAMP since all variables in sys_partman are timestamptz
...
def get_partman_schema(conn):
    cur = conn.cursor()
    sql = "SELECT nspname FROM sys_catalog.sys_namespace n, sys_catalog.sys_extension e WHERE e.extname = 'sys_partman' AND e.extnamespace = n.oid"
    cur.execute(sql)
...
def get_vacuum_freeze_min_age(conn):
    ...
    if args.vacuum_freeze_min_age == None:
        sql = "SELECT setting FROM sys_settings WHERE name = 'vacuum_freeze_min_age'"
        cur.execute(sql)
