...
SELECT sys_catalog.sys_extension_config_dump('part_config', '');
...
SELECT sys_catalog.sys_extension_config_dump('part_config_sub', '');
...
SELECT sys_catalog.sys_extension_config_dump('custom_time_partitions', '');

/*
 * Custom view to help improve privilege lookups for sys_partman. 
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.check_automatic_maintenance_value (p_automatic_maintenance text) RETURNS boolean
    LANGUAGE plsql IMMUTABLE SECURITY DEFINER
    AS $$
DECLARE
v_result    boolean;
BEGIN
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.check_epoch_type (p_type text) RETURNS boolean
    LANGUAGE plsql IMMUTABLE SECURITY DEFINER
    AS $$
DECLARE
v_result    boolean;
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.check_partition_type (p_type text) RETURNS boolean
    LANGUAGE plsql IMMUTABLE SECURITY DEFINER
    AS $$
DECLARE
v_result    boolean;
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.apply_cluster(p_parent_schema text, p_parent_tablename text, p_child_schema text, p_child_tablename text) RETURNS void
    LANGUAGE plsql
AS $$
DECLARE
...
* Adapted from code fork by https://github.com/dturon/sys_partman
*/
--p_parent_schema = upper(p_parent_schema);
--p_parent_tablename = upper(p_parent_tablename);
--p_child_schema = upper(p_child_schema);
--p_child_tablename = upper(p_child_tablename);
...
SELECT substring(sys_get_indexdef(i.indexrelid) from ' USING .*$') AS index_def
...
FOR v_row IN
    ...
    SELECT substring(sys_get_indexdef(i.indexrelid) from ' USING .*$') AS child_indexdef
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.apply_constraints(p_parent_table text, p_child_table text DEFAULT NULL, p_analyze boolean DEFAULT FALSE, p_job_id bigint DEFAULT NULL, p_debug boolean DEFAULT FALSE) RETURNS void
    LANGUAGE plsql
    AS $$
DECLARE
 ...
 */
--p_parent_table = upper(p_parent_table);
--p_child_table = upper(p_child_table);
...
SELECT tablename FROM sys_catalog.sys_tables INTO v_child_exists WHERE schemaname = v_parent_schema::name AND tablename = v_child_tablename::name;
...
LOOP
    ...
    WHERE c.relname = v_child_tablename::name
        ...
        AND ARRAY[a.attnum] OPERATOR(sys_catalog.<@) con.conkey
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.apply_foreign_keys(p_parent_table text, p_child_table text, p_job_id bigint DEFAULT NULL, p_debug boolean DEFAULT false) RETURNS void
    LANGUAGE plsql
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.apply_privileges(p_parent_schema text, p_parent_tablename text, p_child_schema text, p_child_tablename text, p_job_id bigint DEFAULT NULL) RETURNS void
...
SELECT sys_get_userbyid(c.relowner) INTO v_parent_owner 
FROM sys_catalog.sys_class c
JOIN sys_catalog.sys_namespace n ON c.relnamespace = n.oid
WHERE n.nspname = p_parent_schema::name 
AND c.relname = p_parent_tablename::name;

SELECT sys_get_userbyid(c.relowner) INTO v_child_owner
FROM sys_catalog.sys_class c
JOIN sys_catalog.sys_namespace n ON c.relnamespace = n.oid
WHERE n.nspname = p_child_schema::name
AND c.relname = p_child_tablename::name;
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.apply_publications(p_parent_table text, p_child_schema text, p_child_tablename text) RETURNS void
    LANGUAGE plsql 
AS $$
DECLARE
    v_publications      text[];
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.autovacuum_off(p_parent_schema text, p_parent_tablename text, p_source_schema text DEFAULT NULL, p_source_tablename text DEFAULT NULL) RETURNS boolean
    LANGUAGE plsql
    AS $$
DECLARE
...
--p_parent_schema = upper(p_parent_schema);
--p_parent_tablename = upper(p_parent_tablename);
--p_source_schema = upper(p_source_schema);
--p_source_tablename = upper(p_source_tablename);
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.autovacuum_reset(p_parent_schema text, p_parent_tablename text, p_source_schema text DEFAULT NULL, p_source_tablename text DEFAULT NULL) RETURNS boolean
    LANGUAGE plsql
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.check_control_type(p_parent_schema text, p_parent_tablename text, p_control text) RETURNS TABLE (general_type text, exact_type text) 
    LANGUAGE sql STABLE
AS $$
/* 
...
SELECT CASE 
    ...
    JOIN sys_catalog.sys_attribute a ON t.oid = a.atttypid
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.check_name_length (p_object_name text, p_suffix text DEFAULT NULL, p_table_partition boolean DEFAULT FALSE) RETURNS text
    LANGUAGE plsql IMMUTABLE SECURITY DEFINER
    AS $$
DECLARE
    v_new_length    int;
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.check_parent(p_exact_count boolean DEFAULT true) RETURNS SETOF @extschema@.check_parent_table
    LANGUAGE plsql STABLE
    SET search_path = @extschema@,sys_temp
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.check_subpartition_limits(p_parent_table text, p_type text, OUT sub_min text, OUT sub_max text) RETURNS record
    LANGUAGE plsql
    AS $$
DECLARE
...
BEGIN
/*
 * Check if parent table is a subpartition of an already existing partition set managed by sys_partman
...
WITH top_oid AS (
    ...
    FROM sys_catalog.sys_class c
    JOIN sys_catalog.sys_inherits i ON c.oid = i.inhrelid
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.create_function_id(p_parent_table text, p_job_id bigint DEFAULT NULL) RETURNS void
    LANGUAGE plsql
    AS $$
DECLARE
 ...
 */
--p_parent_table = upper(p_parent_table);
--v_higher_parent_schema = upper(v_higher_parent_schema);
--v_higher_parent_table = upper(v_higher_parent_table);
--v_top_parent = upper(v_top_parent);
...
v_trig_func := format('CREATE OR REPLACE INTERNAL FUNCTION %I.%I() RETURNS trigger LANGUAGE plsql AS $t$ 
    DECLARE
        v_count                     int;
        v_current_partition_id      bigint;
        v_current_partition_name    text;
    ...
    IF TG_OP = ''INSERT'' THEN 
        ...
        SELECT count(*) INTO v_count FROM sys_catalog.sys_tables WHERE schemaname = v_parent_schema::name AND tablename = v_current_partition_name::name;
        IF v_count > 0 THEN
    ...
    v_trig_func := v_trig_func ||format('
        ELSE
            ...
            SELECT count(*) INTO v_count FROM sys_catalog.sys_tables WHERE schemaname = %L::name AND tablename = v_current_partition_name::name;
            IF v_count > 0 THEN 
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.create_function_time(p_parent_table text, p_job_id bigint DEFAULT NULL) RETURNS void
    LANGUAGE plsql 
    AS $$
DECLARE
...
ELSIF v_type = 'time-custom' THEN
    ...
    v_trig_func := v_trig_func || format(' 
        ...
        WHERE c.partition_range @> %s::timestamptz 
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.create_parent(
    p_parent_table text
    , p_control text
    , p_type text
    , p_interval text
...
--v_higher_parent_schema = upper(v_higher_parent_schema);
--v_higher_parent_table = upper(v_higher_parent_table);
--v_top_parent_schema = upper(v_top_parent_schema);
--v_top_parent_table = upper(v_top_parent_table);
--p_control = upper(p_control);
...
IF p_publications IS NOT NULL THEN
    ...
    LOOP
        SELECT pubname INTO v_publication_exists FROM sys_catalog.sys_publication where pubname = v_row.pubname::name;
...
LOOP
    RAISE DEBUG 'insert into part_config_sub, v_parent_tablename is %, v_parent_schema is %, p_parent_table is %',v_parent_tablename, v_parent_schema, p_parent_table;
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.create_partition_id(p_parent_table text, p_partition_ids bigint[], p_analyze boolean DEFAULT true, p_debug boolean DEFAULT false) RETURNS boolean
    LANGUAGE plsql 
    AS $$
DECLARE
...
RAISE DEBUG 'v_control is %, v_partition_type is %, v_partition_interval is %, v_inherit_fk is %, v_jobmon is %, v_template_table is %, v_publications is %',v_control, v_partition_type, v_partition_interval, v_inherit_fk, v_jobmon, v_template_table, v_publications;
...
RAISE DEBUG 'v_sub_id_min is %, v_sub_id_max is %',v_sub_id_min, v_sub_id_max;
...
-- Do not create the child table if it's outside the bounds of the top parent. 
    ...
    RAISE DEBUG 'v_partition_name is %',v_partition_name;
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.create_partition_time(p_parent_table text, p_partition_times timestamptz[], p_analyze boolean DEFAULT true, p_debug boolean DEFAULT false) 
RETURNS boolean
    LANGUAGE plsql
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.create_sub_parent(
    p_top_parent text
    , p_control text
    , p_type text
    , p_interval text
 ...
 */
--p_top_parent = upper(p_top_parent);
--p_control = upper(p_control);
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.create_trigger(p_parent_table text) RETURNS void
    LANGUAGE plsql
    AS $$
DECLARE
...
/*
 * Drop constraints managed by sys_partman
 */
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.drop_constraints(p_parent_table text, p_child_table text, p_debug boolean DEFAULT false) RETURNS void
    LANGUAGE plsql
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.drop_partition_column(p_parent_table text, p_column text) RETURNS void
    LANGUAGE plsql
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.drop_partition_id(p_parent_table text, p_retention bigint DEFAULT NULL, p_keep_table boolean DEFAULT NULL, p_keep_index boolean DEFAULT NULL, p_retention_schema text DEFAULT NULL) RETURNS int
    LANGUAGE plsql
    AS $$
DECLARE
 ...
 */
--p_parent_table = upper(p_parent_table);
--p_retention_schema = upper(p_retention_schema);
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.drop_partition_time(p_parent_table text, p_retention interval DEFAULT NULL, p_keep_table boolean DEFAULT NULL, p_keep_index boolean DEFAULT NULL, p_retention_schema text DEFAULT NULL) RETURNS int
    LANGUAGE plsql
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.inherit_template_properties (p_parent_table text, p_child_schema text, p_child_tablename text) RETURNS boolean
    LANGUAGE plsql 
    AS $$
DECLARE
...
FROM sys_catalog.sys_class c
JOIN sys_catalog.sys_namespace n ON c.relnamespace = n.oid
LEFT OUTER JOIN sys_catalog.sys_tablespace ts ON c.reltablespace = ts.oid
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.partition_data_id(p_parent_table text
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.partition_data_time(
        p_parent_table text
        , p_batch_count int DEFAULT 1
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.reapply_privileges(p_parent_table text) RETURNS void
    LANGUAGE plsql 
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.run_maintenance(p_parent_table text DEFAULT NULL, p_analyze boolean DEFAULT NULL, p_jobmon boolean DEFAULT true, p_debug boolean DEFAULT false) RETURNS void 
    LANGUAGE plsql 
    AS $$
DECLARE
...
LOOP
    ...
    RAISE DEBUG 'v_parent_exists is %',v_parent_exists;
    ...
    RAISE DEBUG 'v_sub_parent is %',v_sub_parent;
	...
	RAISE DEBUG 'v_check_subpart is %',v_check_subpart;
    ...
    RAISE DEBUG 'v_analyze IS %',v_analyze;
    ...
    RAISE DEBUG 'v_parent_schema is %, v_parent_tablename is %',v_parent_schema,v_parent_tablename;
    ...
    IF v_row.partition_type = 'native' AND current_setting('server_version_num')::int >= 110000 THEN
        ...
        SELECT sys_get_expr(relpartbound, v_row.parent_table::regclass) INTO v_is_default 
        FROM sys_catalog.sys_class c
        JOIN sys_catalog.sys_namespace n on c.relnamespace = n.oid
        WHERE n.nspname = v_parent_schema
    ...
    RAISE DEBUG 'v_default_tablename is %',v_default_tablename;
    ...
    RAISE DEBUG 'v_control_type is %',v_control_type;
    ...
    RAISE DEBUG 'v_partition_expression is %',v_partition_expression;
    ...
    RAISE DEBUG 'v_last_partition is %',v_last_partition;
	...
	RAISE DEBUG 'v_drop_count is %',v_drop_count;
	    ...
	    RAISE DEBUG 'v_last_partition_created is %',v_last_partition_created;
            IF v_last_partition_created THEN
                v_create_count := v_create_count + 1;
                IF v_row.partition_type <> 'native' THEN
                    PERFORM @extschema@.create_function_id(v_row.parent_table, v_job_id);
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.show_partition_info(p_child_table text
...
IF p_parent_table IS NULL THEN
    ...
    FROM sys_catalog.sys_inherits h
    JOIN sys_catalog.sys_class c ON c.oid = h.inhparent
    JOIN sys_catalog.sys_namespace n ON c.relnamespace = n.oid
...
IF v_control IS NULL THEN
    RAISE EXCEPTION 'Parent table of given child not managed by sys_partman: %', v_parent_table;
END IF;
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.show_partition_name(p_parent_table text, p_value text, OUT partition_schema text, OUT partition_table text, OUT suffix_timestamp timestamptz, OUT suffix_id bigint, OUT table_exists boolean) RETURNS record
    LANGUAGE plsql STABLE
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.show_partitions (p_parent_table text, p_order text DEFAULT 'ASC', p_include_default boolean DEFAULT false) RETURNS TABLE (partition_schemaname text, partition_tablename text)
    LANGUAGE plsql STABLE
    SET search_path = @extschema@,sys_temp
    AS $$
DECLARE
...
IF v_partition_type IS NULL THEN
    RAISE EXCEPTION 'Given parent table not managed by sys_partman: %', p_parent_table;
END IF;
...
IF v_partition_type = 'native' AND current_setting('server_version_num')::int >= 110000 THEN
    ...
    IF p_include_default THEN
        ...
        v_default_sql := v_sql || format('
            AND sys_get_expr(relpartbound, c.oid) = ''DEFAULT''');
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.stop_sub_partition(p_parent_table text, p_jobmon boolean DEFAULT true) RETURNS boolean
    LANGUAGE plsql 
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION @extschema@.undo_partition(p_parent_table text, p_batch_count int DEFAULT 1, p_batch_interval text DEFAULT NULL, p_keep_table boolean DEFAULT true, p_lock_wait numeric DEFAULT 0, p_target_table text DEFAULT NULL, OUT partitions_undone int, OUT rows_undone bigint) RETURNS record
    LANGUAGE plsql 
    AS $$
DECLARE
 ...
 */
--p_parent_table = upper(p_parent_table);
--p_target_table = upper(p_target_table);
...
IF v_control IS NULL THEN
    RAISE EXCEPTION 'No configuration found for sys_partman for given parent table: %', p_parent_table;
END IF;
...
IF v_partition_type != 'native' THEN
    ...
    FROM sys_catalog.sys_trigger t
    JOIN sys_catalog.sys_class c ON t.tgrelid = c.oid
    WHERE tgname = v_trig_name::name 
    AND c.relname = v_parent_tablename::name;

    SELECT proname INTO v_function_name FROM sys_catalog.sys_proc p JOIN sys_catalog.sys_namespace n ON p.pronamespace = n.oid WHERE n.nspname = v_parent_schema::name AND proname = v_function_name::name;
...
END IF; -- end sys_partman trigger cleanup
