-- Foreign keys placed on the parent table are now inherited to child tables.
    ...
    -- The new apply_foreign_keys() function & reapply_foreign_keys.py script can be applied to any table inheritance set, not just the ones managed by sys_partman.
...
-- Added lockwait options to the undo partition functions (plsql & python)
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION apply_foreign_keys(p_parent_table text, p_child_table text DEFAULT NULL, p_debug boolean DEFAULT false) RETURNS void
    LANGUAGE plsql
    AS $$
DECLARE
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION undo_partition_id(p_parent_table text, p_batch_count int DEFAULT 1, p_batch_interval bigint DEFAULT NULL, p_keep_table boolean DEFAULT true, p_lock_wait numeric DEFAULT 0) RETURNS bigint
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION undo_partition_time(p_parent_table text, p_batch_count int DEFAULT 1, p_batch_interval interval DEFAULT NULL, p_keep_table boolean DEFAULT true, p_lock_wait numeric DEFAULT 0) RETURNS bigint
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION undo_partition(p_parent_table text, p_batch_count int DEFAULT 1, p_keep_table boolean DEFAULT true, p_jobmon boolean DEFAULT true, p_lock_wait numeric DEFAULT 0) RETURNS bigint
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
