-- IMPORTANT NOTE: The initial version of sys_partman 3 had to be split into two updates due to changing both the data in the config table as well as adding constraints on that data. Depending on the data contained in the config table, doing this in a single-transaction update may not work (you may see an error about pending trigger events). If this is the case, please update to version 3.0.1 in a separate transaction from your update to 3.0.0. Example:
    
    /*
        BEGIN;
        ALTER EXTENSION sys_partman UPDATE TO '3.0.0';
        COMMIT;

        BEGIN;
        ALTER EXTENSION sys_partman UPDATE TO '3.0.1';
        COMMIT;
    */

-- Update 3.0.1 MUST be installed for version 3.x of sys_partman to work properly. As long as these updates are run within a few seconds of each other, there should be no issues.
...
-- Features NOT (yet) available in native:
    -- Indexes are not currently possible to create on a partitioned parent table. This means there is currently no automatic index support for native partitioning in sys_partman. Waiting to see what final implementation will be like before adding code to handle this.
...
-- Removed automatic partition creation via trigger at 50% for id/serial partitioning. Was making trigger function generation code harder to maintain and is just a bad practice in general to be making child partitions on the fly with data insertion. This is also not possible with native partitioning, so rather phase out the feature now. Now must use automatic partitioning and schedule frequently enough to keep up with ingestion rate or manually call run_maintenance() via some other means to maintain new partition creation for id-based partition sets.
    ...
    -- If you encounter any issues caused by this update and the removal of the 50% code, call create_function_id() to recreate the trigger. This function has been added to sys_partman.md for reference.
...
-- The undo_partition plsql functions now return both the number of partitions undone as well as the number of rows copied/moved. Result is returned as a record set, so these functions can be queried as a table to have columns/rows returned. The value of undone partitions will be -1 when an issue is encountered while running the function.
...
ALTER TABLE @extschema@.part_config ALTER COLUMN automatic_maintenance TYPE text;
ALTER TABLE @extschema@.part_config_sub ALTER COLUMN sub_automatic_maintenance TYPE text;

UPDATE @extschema@.part_config SET automatic_maintenance = 'on';
UPDATE @extschema@.part_config_sub SET sub_automatic_maintenance = 'on';

CREATE TEMP TABLE partman_preserve_privs_temp (statement text);

INSERT INTO partman_preserve_privs_temp 
SELECT 'GRANT EXECUTE ON FUNCTION @extschema@.create_parent(text, text, text, text, text[], int, text, text, boolean, text, text, boolean, boolean, boolean) TO '||array_to_string(array_agg(grantee::text), ',')||';' 
FROM information_schema.routine_privileges
WHERE routine_schema = '@extschema@'
AND routine_name = 'create_parent'; 

INSERT INTO partman_preserve_privs_temp 
SELECT 'GRANT EXECUTE ON FUNCTION @extschema@.create_sub_parent(text, text, text, text, text, text[], int, text, boolean, text, text, boolean, boolean, boolean) TO '||array_to_string(array_agg(grantee::text), ',')||';' 
FROM information_schema.routine_privileges
WHERE routine_schema = '@extschema@'
AND routine_name = 'create_sub_parent'; 

INSERT INTO partman_preserve_privs_temp 
SELECT 'GRANT EXECUTE ON FUNCTION @extschema@.check_subpartition_limits(text, text) TO '||array_to_string(array_agg(grantee::text), ',')||';' 
FROM information_schema.routine_privileges
WHERE routine_schema = '@extschema@'
AND routine_name = 'check_subpartition_limits'; 
...
v_exists    smallint;
BEGIN
    SELECT count(*) FROM sys_catalog.sys_class INTO v_exists
...
CREATE OR REPLACE INTERNAL FUNCTION check_automatic_maintenance_value (p_automatic_maintenance text) RETURNS boolean
    LANGUAGE plsql IMMUTABLE SECURITY DEFINER
    AS $$
DECLARE
v_result    boolean;
...
CREATE OR REPLACE INTERNAL FUNCTION check_control_type(p_parent_schema text, p_parent_tablename text, p_control text) RETURNS TABLE (general_type text, exact_type text) 
...
CREATE OR REPLACE INTERNAL FUNCTION create_function_id(p_parent_table text, p_job_id bigint DEFAULT NULL) RETURNS void
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION stop_sub_partition(p_parent_table text, p_jobmon boolean DEFAULT true) RETURNS boolean
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION undo_partition(p_parent_table text, p_batch_count int DEFAULT 1, p_keep_table boolean DEFAULT true, p_jobmon boolean DEFAULT true, p_lock_wait numeric DEFAULT 0, OUT partitions_undone int, OUT rows_undone bigint) RETURNS record 
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 * Will actually work on any non-native parent/child table set, not just ones created by sys_partman.
 */

v_adv_lock := sys_try_advisory_xact_lock(hashtext('sys_partman undo_partition'));
IF v_adv_lock = 'false' THEN
...
WHILE v_batch_loop_count < p_batch_count LOOP 
    ...
    -- Not using show_partitions() so it can work on non-sys_partman partition sets
    WITH parent_info AS (
        SELECT c1.oid 
        FROM sys_catalog.sys_class c1 
        JOIN sys_catalog.sys_namespace n1 ON c1.relnamespace = n1.oid
    ...
    FROM sys_catalog.sys_inherits i
    JOIN sys_catalog.sys_class c ON i.inhrelid = c.oid
...
IF v_undo_count = 0 THEN
    ...
    IF v_jobmon_schema IS NOT NULL THEN
        v_step_id := add_step(v_job_id, 'Removing config from sys_partman (if it existed)');
        PERFORM update_step(v_step_id, 'OK', 'Done');
    END IF;
END IF;
...
CREATE OR REPLACE INTERNAL FUNCTION undo_partition_id(p_parent_table text, p_batch_count int DEFAULT 1, p_batch_interval bigint DEFAULT NULL, p_keep_table boolean DEFAULT true, p_lock_wait numeric DEFAULT 0, OUT partitions_undone int, OUT rows_undone bigint) RETURNS record 
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
v_adv_lock := sys_try_advisory_xact_lock(hashtext('sys_partman undo_partition_id'));
IF v_adv_lock = 'false' THEN
...
IF v_control IS NULL THEN
    RAISE EXCEPTION 'No non-native configuration found for sys_partman for given parent table: %', p_parent_table;
END IF;
...
v_partition_interval := v_partition_int_text::bigint;
...
CREATE OR REPLACE INTERNAL FUNCTION undo_partition_time(p_parent_table text, p_batch_count int DEFAULT 1, p_batch_interval interval DEFAULT NULL, p_keep_table boolean DEFAULT true, p_lock_wait numeric DEFAULT 0, OUT partitions_undone int, OUT rows_undone bigint) RETURNS record
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
CREATE OR REPLACE INTERNAL FUNCTION undo_partition_native(p_parent_table text, p_target_table text, p_batch_count int DEFAULT 1, p_batch_interval text DEFAULT NULL, p_keep_table boolean DEFAULT true, p_lock_wait numeric DEFAULT 0, OUT partitions_undone int, OUT rows_undone bigint) RETURNS record
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
