...
-- Changed the manner in which new partition creation is logged in sys_jobmon. Previously, each individual child table creation was logged as its own, separate job entry in sys_jobmon. Now, if multiple child partitions are created for a single partition set, all those child tables are logged as steps for a single job log entry. This now allows the analyze step (if it is done) to be logged as well in sys_jobmon and allows for easier diagnosis if this if holding up partition maintenance.
