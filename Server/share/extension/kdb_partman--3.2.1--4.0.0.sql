...
-- Note that if this version is installed on PG10 and older, the procedures will not be created. A migration script to update sys_partman to include those will be made available once PG11 stable has been released.

-- Major change in privilege/security of this extension. SECURITY DEFINER is no longer in use to cause most of sys_partman's functions to run as the owner of the called function. This is mostly due to the requirements to support PROCEDUREs in PG11, but it's also generally a better idea for security in the long run.
    -- It is recommended to create a dedicated role for sys_partman maintenance runs and give it the privileges outlined in the setup section of the top level README.
    ...
    -- The role running maintenance must now always be the owner of the tables it is managing. This shouldn't be too drastic of a change since previously it was making the maintenace function owner the owner of the partitioned tables. However, if you've changed partition table ownership after initial creation, you may run into issues until you set the sys_partman role as the owner.
    ...
    -- Two functions retain their SECURITY DEFINER status: 
        ...
        -- apply_privileges() - Requires access to sys_authid & also potentially changes object ownership.
    -- If you're using sys_jobmon, privileges for that extension are managed independently of sys_partman. See that extension's documentation for privileges it requires.    
...
-- The old undo_partition() function that just copied data from child tables to the parent has been removed. The only utility this really had was a possible way to support undoing partition sets not maintained by sys_partman. With native partitioning now in place, its continued usability is now questionable and it was also causing confusion since people were using it and not the previous ones specialized for sys_partman. Can incorporate back in if someone can provide a further beneficial use case.
...
-- For PG11+, a DEFAULT partition is automatically created as part of the partition set to handle any data that does not have a matching child table. 
    ...
    -- Support within sys_partman for moving data out of the default partition is not included in 4.0.0, but will be included in a future release.
...
-- For PostgreSQL 11+, run_maintenance_proc() has been added and should cause much less contention when running against many large partition sets in a single call.
    -- For PG11+, this is the preferred maintenance method to run over the old run_maintenance() function. The old one will continue to work for the next few major releases, but please update all sys_partman maintenence calls in your environment to use the new procedure.
...
v_adv_lock := sys_try_advisory_xact_lock(hashtext('sys_partman partition_data_proc'), hashtext(p_parent_table));
...
v_adv_lock := sys_try_advisory_xact_lock(hashtext('sys_partman undo_partition_proc'), hashtext(p_parent_table));
IF v_adv_lock = 'false' THEN
    RAISE NOTICE 'Partman partition_data_proc already running for given parent table: %.', p_parent_table;
    RETURN;
END IF;
...
v_adv_lock := sys_try_advisory_lock(hashtext('sys_partman run_maintenance'));
IF v_adv_lock = false THEN
...
PERFORM sys_advisory_unlock(hashtext('sys_partman run_maintenance'));
END
$$;
$run_maintenance$;
...
BEGIN
/*
 * Procedure for reapplying additional constraints managed by sys_partman on child tables. See docs for additional info on this special constraint management. 
...
v_adv_lock := sys_try_advisory_lock(hashtext('sys_partman reapply_constraints'));
IF v_adv_lock = false THEN
...
IF v_premake IS NULL THEN
    RAISE EXCEPTION 'Unable to find given parent in sys_partman config: %. This procedure is only meant to be called on sys_partman managed partition sets.', p_parent_table;
END IF;
...
PERFORM sys_advisory_unlock(hashtext('sys_partman reapply_constraints'));
END
$$;
$reapply_constraints$;
