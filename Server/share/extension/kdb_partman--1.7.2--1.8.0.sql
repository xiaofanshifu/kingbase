 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION check_subpart_sameconfig(text) RETURNS boolean
    LANGUAGE sql STABLE
    AS $$
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_partition_id(p_parent_table text, p_partition_ids bigint[], p_analyze boolean DEFAULT true) RETURNS boolean
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_partition_time (p_parent_table text, p_partition_times timestamp[], p_analyze boolean DEFAULT true) 
RETURNS boolean
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
-- Check if parent table is a subpartition of an already existing time-based partition set managed by sys_partman
-- If so, limit what child tables can be created based on parent suffix
WITH top_oid AS (
    SELECT i.inhparent AS top_parent_oid
    FROM sys_catalog.sys_class c
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_function_time(p_parent_table text) RETURNS void
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_function_id(p_parent_table text) RETURNS void
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION run_maintenance(p_parent_table text DEFAULT NULL, p_analyze boolean DEFAULT true, p_jobmon boolean DEFAULT true) RETURNS void 
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
