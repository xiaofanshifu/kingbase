 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION check_name_length (p_object_name text, p_object_schema text DEFAULT NULL, p_suffix text DEFAULT NULL, p_table_partition boolean DEFAULT FALSE) RETURNS text
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION check_unique_column(p_parent_table text, p_column text) RETURNS SETOF check_unique_table
    LANGUAGE plsql
    AS $$
DECLARE
...
SELECT tableowner, schemaname, tablename INTO v_parent_owner, v_parent_schema, v_parent_tablename FROM sys_tables WHERE schemaname ||'.'|| tablename = p_parent_table;
...
IF v_last_partition IS NOT NULL THEN
    SELECT tablename INTO v_tablename FROM sys_tables WHERE schemaname || '.' || tablename = v_last_partition;
...
v_adv_lock := sys_try_advisory_lock(hashtext('sys_partman undo_time_partition'));
IF v_adv_lock = 'false' THEN
...
PERFORM sys_advisory_unlock(hashtext('sys_partman undo_time_partition'));
...
v_adv_lock := sys_try_advisory_lock(hashtext('sys_partman undo_id_partition'));
IF v_adv_lock = 'false' THEN
...
PERFORM sys_advisory_unlock(hashtext('sys_partman undo_id_partition'));
...
v_adv_lock := sys_try_advisory_lock(hashtext('sys_partman undo_partition'));
IF v_adv_lock = 'false' THEN
...
PERFORM sys_advisory_unlock(hashtext('sys_partman undo_partition'));
...
v_adv_lock := sys_try_advisory_lock(hashtext('sys_partman drop_partition_id'));
IF v_adv_lock = 'false' THEN
...
PERFORM sys_advisory_unlock(hashtext('sys_partman drop_partition_id'));
...
v_adv_lock := sys_try_advisory_lock(hashtext('sys_partman drop_partition_time'));
IF v_adv_lock = 'false' THEN
...
PERFORM sys_advisory_unlock(hashtext('sys_partman drop_partition_time'));
