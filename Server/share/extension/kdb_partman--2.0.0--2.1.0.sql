...
-- New function 'show_partition_name()' can tell you the child table name that a given value would exist in given a parent table that sys_partman manages. The name will always be returned whether the child table exists or not. Another boolean column is returned that tells you whether the child table actually does exist. It also returns the raw value (timestamp or integer) for the suffix of the returned partition name (used internally, but could be useful elsewhere too). Thanks to Corey Huinker for idea & assistance.
...
-- Bumped all minimum sys_partman version requirements for python scripts to 2.0.0. May still work on the older versions, but not officially supported or guaranteed anymore.
-- New --nonpartman option to the reapply_indexes.py and reapply_foreign_keys.py scripts to allow them to work better with partition sets not managed by sys_partman and to allow those that are to work more efficiently.
-- Greatly reduced number of individual sys_jobmon jobs generated. Steps such as creating partition functions, applying foreign keys, and applying additional constraints have been combined into the job log entries for creating partitions or general maintenance when applicable.
...
-- NOTE: If you are running version 1.8.7 and need to update to 1.8.8 do the following steps:
    ...
    -- Copy the file "updates/sys_partman--1.8.7--1.8.8.sql" to the folder where your extension SQL files are kept (depends on your OS or where you manually installed postgres).
    ...
    -- While logged into postgres run: ALTER EXTENSION sys_partman UPDATE TO '1.8.8';
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION check_name_length (p_object_name text, p_suffix text DEFAULT NULL, p_table_partition boolean DEFAULT FALSE) RETURNS text
...
LOOP
    SELECT schemaname, tablename INTO v_schemaname, v_tablename FROM sys_catalog.sys_tables WHERE schemaname ||'.'||tablename = v_row.parent_table;
...
SELECT schemaname, tablename INTO v_child_schemaname, v_child_tablename FROM sys_catalog.sys_tables WHERE schemaname||'.'||tablename = p_child_table;
...
SELECT schemaname, tablename, tableowner INTO v_parent_schema, v_parent_tablename, v_parent_owner FROM sys_catalog.sys_tables WHERE schemaname ||'.'|| tablename = p_parent_table;
...
LOOP
    ...
    SELECT tableowner INTO v_child_owner FROM sys_tables WHERE schemaname = v_row.partition_schemaname AND tablename = v_row.partition_tablename;
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION show_partition_name(p_parent_table text, p_value text, OUT partition_table text, OUT suffix_timestamp timestamp, OUT suffix_id bigint, OUT table_exists boolean) RETURNS record
...
SELECT schemaname, tablename INTO v_child_exists FROM sys_catalog.sys_tables WHERE schemaname ||'.'|| tablename = partition_table;
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION show_partitions (p_parent_table text, p_order text DEFAULT 'ASC') RETURNS TABLE (partition_schemaname text, partition_tablename text)
    LANGUAGE plsql STABLE SECURITY DEFINER 
    AS $$
DECLARE
