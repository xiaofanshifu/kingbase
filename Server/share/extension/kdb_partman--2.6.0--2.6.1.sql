-- WARNING: The sys_partman background worker is currently incompatible with PostgreSQL 9.6. It runs fine in 9.4 and 9.5. I'm still working on trying to figure out what the issue is. If you must upgrade to 9.6 before this issue is fixed, you can disable the background worker by removing the entry for it in shared_preload_libraries and restarting postgres. Instead schedule the run_maintenance() plsql function to run via another scheduler method (cron).
...
-- Account for non-lowercase schemename for sys_partman's schema in background worker (Github PullRequest #140).
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_function_id(p_parent_table text, p_job_id bigint DEFAULT NULL, p_analyze boolean DEFAULT true) RETURNS void
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION partition_data_id(p_parent_table text
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION partition_data_time(
        p_parent_table text
        , p_batch_count int DEFAULT 1
