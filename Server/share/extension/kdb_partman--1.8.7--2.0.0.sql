-- IMPORTANT NOTE: This version, and all future versions of sys_partman >= 2.0.0, are only compatible with PostgreSQL 9.4 and greater.
...
-- A background worker process (BGW) has been added to sys_partman for general partition maintenance.
    -- A separate scheduler is no longer required in most cases if you compile sys_partman with the background worker option.
...
/***********************************************************
-- Begin code to migrate existing partition sets to new hybrid function
...
CREATE TEMP TABLE partman_migrate_hybrid_temp (statement text);

INSERT INTO partman_migrate_hybrid_temp 
SELECT format('SELECT create_function_id(%L);', parent_table)
FROM part_config
WHERE partition_type = 'id';

INSERT INTO partman_migrate_hybrid_temp 
SELECT format('SELECT create_function_time(%L);', parent_table)
FROM part_config
WHERE partition_type = 'time';

DO $$
DECLARE
v_row   record;
BEGIN
    FOR v_row IN SELECT statement FROM partman_migrate_hybrid_temp LOOP
...
DROP TABLE IF EXISTS partman_migrate_hybrid_temp;

-- End code to migrate existing partition sets to new hybrid function
************************************************************/
...
SELECT tablename INTO v_tablename FROM sys_tables WHERE schemaname || '.' || tablename = p_parent_table;
    IF v_tablename IS NULL THEN
        RAISE EXCEPTION 'Please create given parent table first: %', p_parent_table;
    END IF;

SELECT attnotnull INTO v_notnull FROM sys_attribute WHERE attrelid = p_parent_table::regclass AND attname = p_control;
...
IF v_jobmon_schema IS NOT NULL THEN
    v_job_id := add_job('PARTMAN CREATE OR REPLACE INTERNAL FUNCTION: '||p_parent_table);
...
v_trig_func := 'CREATE OR REPLACE FUNCTION '||v_function_name||'() RETURNS trigger LANGUAGE plsql AS $t$ 
    DECLARE
        v_count                     int;
        v_current_partition_id      bigint;
        v_current_partition_name    text;
    ...
    IF TG_OP = ''INSERT'' THEN 
        ...
        SELECT count(*) INTO v_count FROM sys_catalog.sys_tables WHERE schemaname ||'.'||tablename = v_current_partition_name;
        IF v_count > 0 THEN
    ...
    FOR i IN 1..v_premake LOOP
        ...
        SELECT count(*) INTO v_count FROM sys_catalog.sys_tables WHERE schemaname ||'.'||tablename = v_prev_partition_name;
        IF v_count > 0 THEN
    ...
    v_trig_func := v_trig_func ||'
        ELSE
            ...
            v_current_partition_name := @extschema@.check_name_length('''||v_parent_tablename||''', '''||v_parent_schema||''', v_current_partition_id::text, TRUE);
            SELECT count(*) INTO v_count FROM sys_tables WHERE schemaname ||''.''|| tablename = v_current_partition_name;
            IF v_count > 0 THEN 
...
IF v_type = 'time' THEN
    ...
    v_trig_func := v_trig_func||'
            ELSE
                ...
                SELECT count(*) INTO v_count FROM sys_tables WHERE schemaname ||''.''|| tablename = v_partition_name;
                IF v_count > 0 THEN 
...
ELSIF v_type = 'time-custom' THEN
    ...
    v_trig_func := 'CREATE OR REPLACE FUNCTION '||v_function_name||'() RETURNS trigger LANGUAGE plsql AS $t$ 
        ...
        SELECT count(*) INTO v_count FROM sys_tables WHERE schemaname ||''.''|| tablename = v_child_table;
        IF v_count > 0 THEN
...
-- Do not create the child table if it's outside the bounds of the top parent. 
    ...
    SELECT tablename INTO v_tablename FROM sys_catalog.sys_tables WHERE schemaname ||'.'|| tablename = v_partition_name;
    ...
    SELECT relpersistence INTO v_unlogged FROM sys_catalog.sys_class WHERE oid::regclass = p_parent_table::regclass;
    v_sql := 'CREATE';
    ...
    SELECT relhasoids INTO v_hasoids FROM sys_catalog.sys_class WHERE oid::regclass = p_parent_table::regclass;
...
LOOP
    ...
    FROM sys_catalog.sys_constraint c 
        JOIN sys_catalog.sys_attribute a ON c.conrelid = a.attrelid 
...
FOR v_row IN 
    ...
    FROM
        ( SELECT unnest(con.conkey) as ref
          ...
          FROM sys_catalog.sys_class c
          JOIN sys_catalog.sys_namespace n ON c.relnamespace = n.oid
          JOIN sys_catalog.sys_constraint con ON c.oid = con.conrelid
          WHERE n.nspname ||'.'|| c.relname = p_parent_table
          AND con.contype = 'f'
          ORDER BY con.conkey
    ) keys
    JOIN sys_catalog.sys_class cl ON cl.oid = keys.confrelid
    JOIN sys_catalog.sys_namespace n ON cl.relnamespace = n.oid
    JOIN sys_catalog.sys_attribute att ON att.attrelid = keys.confrelid AND att.attnum = keys.child
    JOIN sys_catalog.sys_attribute att2 ON att2.attrelid = keys.conrelid AND att2.attnum = keys.ref
    GROUP BY n.nspname, cl.relname, keys.condeferred, keys.condeferrable, keys.confupdtype, keys.confdeltype, keys.confmatchtype
LOOP
    SELECT schemaname, tablename INTO v_ref_schema, v_ref_table FROM sys_tables WHERE schemaname||'.'||tablename = v_row.ref_table;
...
FOR v_child_table IN 
    SELECT n.nspname||'.'||c.relname FROM sys_inherits i join sys_class c ON i.inhrelid = c.oid join sys_namespace n ON c.relnamespace = n.oid WHERE i.inhparent::regclass = p_parent_table::regclass ORDER BY i.inhrelid ASC
LOOP
        ...
        IF v_retention_schema IS NULL THEN
            ...
            ELSIF v_retention_keep_index = false THEN
                FOR v_index IN 
                    ...
                    LEFT JOIN sys_catalog.sys_constraint c ON i.indexrelid = c.conindid 
...
SELECT tableowner INTO v_parent_owner FROM sys_tables WHERE schemaname ||'.'|| tablename = p_parent_table;
...
LOOP
    ...
    SELECT tableowner INTO v_child_owner FROM sys_tables WHERE schemaname ||'.'|| tablename = v_child_table;
 ...
 * Will actually work on any parent/child table set, not just ones created by sys_partman.
 */
...
WHILE v_batch_loop_count < p_batch_count LOOP 
    ...
    FROM sys_inherits i 
    JOIN sys_class c ON i.inhrelid = c.oid 
...
/*
 * Check if parent table is a subpartition of an already existing id based partition set managed by sys_partman
