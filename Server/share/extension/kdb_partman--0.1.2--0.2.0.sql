 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION drop_id_partition(p_parent_table text, p_keep_table boolean DEFAULT NULL, p_keep_index boolean DEFAULT NULL) RETURNS int
...
v_adv_lock := sys_try_advisory_lock(hashtext('sys_partman drop_id_partition'));
...
PERFORM sys_advisory_unlock(hashtext('sys_partman drop_id_partition'));
 ...
 * Function to drop child tables from a time-based partition set. Options to drop indexes or actually drop the table from the database.
 */
CREATE OR REPLACE INTERNAL FUNCTION drop_time_partition(p_parent_table text, p_keep_table boolean DEFAULT NULL, p_keep_index boolean DEFAULT NULL) RETURNS int
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
v_adv_lock := sys_try_advisory_lock(hashtext('sys_partman drop_time_partition'));
IF v_adv_lock = 'false' THEN
...
PERFORM sys_advisory_unlock(hashtext('sys_partman drop_time_partition'));
