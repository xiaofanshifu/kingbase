...
-- Bug fix: reapply_privileges() had unconditional calls to sys_jobmon functions and would fail if it wasn't installed.
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION drop_partition_time(p_parent_table text, p_retention interval DEFAULT NULL, p_keep_table boolean DEFAULT NULL, p_keep_index boolean DEFAULT NULL) RETURNS int
 ...
 * Function to drop child tables from a time-based partition set. Options to drop indexes or actually drop the table from the database.
 */
CREATE OR REPLACE INTERNAL FUNCTION drop_partition_id(p_parent_table text, p_retention bigint DEFAULT NULL, p_keep_table boolean DEFAULT NULL, p_keep_index boolean DEFAULT NULL) RETURNS int
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
