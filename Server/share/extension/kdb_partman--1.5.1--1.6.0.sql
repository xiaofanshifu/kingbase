...
-- sys_jobmon is now truly optional. Additonal configuration option for each individual partition set to turn it off and on. run_maintenance() now has an optional parameter to turn it off when being run. If you tried to partition sys_jobmon tables before, it would cause a permanent lockwait. Turn sys_jobmon off for those tables to avoid this.
...
-- sys_partman only supports id intervals greater than 1. May see if I can get an interval of 1 working later, but changed create_parent() to check for this and not allow it since it won't work properly at this time. New partitions were not automatically created if interval was set to 1. (Github issue #15)
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION check_version(p_check_version text) RETURNS boolean
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_time_partition (p_parent_table text, p_partition_times timestamp[]) 
RETURNS text
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
EXCEPTION
    WHEN OTHERS THEN
        IF v_jobmon_schema IS NOT NULL THEN
            IF v_job_id IS NULL THEN
                EXECUTE 'SELECT '||v_jobmon_schema||'.add_job(''PARTMAN CREATE OR REPLACE INTERNAL FUNCTION: '||p_parent_table||''')' INTO v_job_id;
                EXECUTE 'SELECT '||v_jobmon_schema||'.add_step('||v_job_id||', ''EXCEPTION before job logging started'')' INTO v_step_id;
            ELSIF v_step_id IS NULL THEN
                EXECUTE 'SELECT '||v_jobmon_schema||'.add_step('||v_job_id||', ''EXCEPTION before first step logged'')' INTO v_step_id;
            END IF;
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION run_maintenance(p_jobmon BOOLEAN DEFAULT true) RETURNS void 
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
LOOP
    ...
    IF v_row.last_partition IS NOT NULL THEN
        SELECT tablename INTO v_tablename FROM sys_tables WHERE schemaname || '.' || tablename = v_row.last_partition;
        IF v_tablename IS NULL THEN
...
v_adv_lock := sys_try_advisory_xact_lock(hashtext('sys_partman undo_time_partition'));
IF v_adv_lock = 'false' THEN
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_id_partition (p_parent_table text, p_partition_ids bigint[]) RETURNS text
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
v_adv_lock := sys_try_advisory_xact_lock(hashtext('sys_partman undo_id_partition'));
IF v_adv_lock = 'false' THEN
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION undo_partition(p_parent_table text, p_batch_count int DEFAULT 1, p_keep_table boolean DEFAULT true, p_jobmon boolean DEFAULT true) RETURNS bigint
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
