...
-- Fixed some calls to sys_jobmon that were outside the checks to see if it's actually installed
-- Properly reset search path back to original before partman functions were run if sys_jobmon is being used
-- Changed the default premake to 4 instead of 3. This will cause sys_jobmon's default monitoring for 3 consecutive failing jobs to trigger an before the last premade partition is used up.
-- Added optional jobmon logging to run_maintenance() so that if it fails, sys_jobmon can notify that maintenance didn't work.
...
IF v_type = 'time-static' THEN
    ...
    v_trig_func := 'CREATE OR REPLACE FUNCTION '||p_parent_table||'_part_trig_func() RETURNS trigger LANGUAGE plsql AS $t$ 
        BEGIN 
...
RAISE NOTICE 'v_max_partition_timestamp: %',v_max_partition_timestamp;
    ...
    -- sys_partman was used to set up partitioning.
