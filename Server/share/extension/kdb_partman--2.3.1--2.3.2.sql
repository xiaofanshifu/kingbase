...
-- Added new options to vacuum_maintenance.py script
    -- New --all option tells it to run against all tables managed by sys_partman.
    -- New --type option sets whether to run against time or id based partition sets managed by sys_partman.
    ...
    -- All these options only work on partition sets managed by sys_partman. If none of these options are set, script still works on non-sys_partman inheritance sets. 
...
-- Set part_config_sub & custom_time_partition config tables to dump their contents with sys_dump to ensure all config data is saved.
...
CREATE OR REPLACE INTERNAL FUNCTION show_partition_info(p_child_table text
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_partition_id(p_parent_table text, p_partition_ids bigint[], p_analyze boolean DEFAULT true, p_debug boolean DEFAULT false) RETURNS boolean
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_partition_time(p_parent_table text, p_partition_times timestamp[], p_analyze boolean DEFAULT true, p_debug boolean DEFAULT false) 
RETURNS boolean
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
