...
-- Applying of grants is now logged in sys_jobmon so if there's any issues with that step, it's clear where it failed.

/*
 * Check function for config table partition types
 */
CREATE OR REPLACE INTERNAL FUNCTION check_partition_type (p_type text) RETURNS boolean
...
SELECT tableowner INTO v_owner FROM sys_tables WHERE schemaname ||'.'|| tablename = p_parent_table;

FOR v_child_table IN 
    SELECT inhrelid::regclass FROM sys_catalog.sys_inherits WHERE inhparent::regclass = p_parent_table::regclass ORDER BY inhrelid::regclass ASC
LOOP
    v_grantees := NULL;
    FOR v_row IN 
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION create_parent(p_parent_table text, p_control text, p_type text, p_interval text, p_premake int DEFAULT 4, p_debug boolean DEFAULT false) RETURNS void
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
