...
-- New python script, dump_partition.py, that will dump any tables found in a given schema using sys_dump, create a SHA-512 hash of the dumped file and then drop the table from the database.
...
CREATE TEMP TABLE sys_partman_preserve_privs_temp (statement text);

INSERT INTO sys_partman_preserve_privs_temp 
...
CREATE OR REPLACE INTERNAL FUNCTION replay_preserved_privs() RETURNS void
    LANGUAGE plsql
    AS $$
DECLARE
v_row   record;
BEGIN
    FOR v_row IN SELECT statement FROM sys_partman_preserve_privs_temp LOOP
 ...
 * not just those managed by sys_partman.
 */
CREATE OR REPLACE INTERNAL FUNCTION show_partitions (p_parent_table text) RETURNS SETOF text 
...
FOREACH v_id IN ARRAY p_partition_ids LOOP
    ...
    SELECT schemaname ||'.'|| tablename INTO v_tablename FROM sys_catalog.sys_tables WHERE schemaname ||'.'|| tablename = v_partition_name;
 ...
 */
CREATE OR REPLACE INTERNAL FUNCTION drop_partition_id(p_parent_table text, p_retention bigint DEFAULT NULL, p_keep_table boolean DEFAULT NULL, p_keep_index boolean DEFAULT NULL, p_retention_schema text DEFAULT NULL) RETURNS int
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 * Options to move table to different schema, drop only indexes or actually drop the table from the database.
 */
CREATE OR REPLACE INTERNAL FUNCTION drop_partition_time(p_parent_table text, p_retention interval DEFAULT NULL, p_keep_table boolean DEFAULT NULL, p_keep_index boolean DEFAULT NULL, p_retention_schema text DEFAULT NULL) RETURNS int
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
DROP TABLE sys_partman_preserve_privs_temp;
