-- New migration documentation (doc/migration.md) to provide assistance changing an existing partition set to be used by sys_partman. Note that sys_partman does not support having child table names that do not comply with its naming convention.
...
-- Added new configuration option to configuration table for time-based partitioning: infinite_time_partitions (Github Pull Request 84)
    -- By default, if no new data is being inserted, no new partitions are created. This keeps sys_partman from creating new, empty tables infinitely when data insertion has stopped.
    -- Setting this value to true in part_config or part_config_sub will tell sys_partman to continue making partitions forever with time-based partitioning, even if there's no new data
    -- Note that serial/id partition is not affected by this and will only make new partitions if new data is being inserted.

-- Implemented the clustering work done in fork by https://github.com/dturon/sys_partman
...
*/
CREATE OR REPLACE INTERNAL FUNCTION apply_cluster(p_parent_schema text, p_parent_tablename text, p_child_schema text, p_child_tablename text) RETURNS void
    LANGUAGE plsql SECURITY DEFINER
AS $$
DECLARE
...
SELECT tablename FROM sys_catalog.sys_tables INTO v_child_exists WHERE schemaname = v_parent_schema AND tablename = v_child_tablename;
