...
SELECT tableowner INTO v_parent_owner FROM sys_catalog.sys_tables WHERE schemaname = p_parent_schema::name AND tablename = p_parent_tablename::name;
SELECT tableowner INTO v_child_owner FROM sys_catalog.sys_tables WHERE schemaname = p_child_schema::name AND tablename = p_child_tablename::name;
...
-- Do not create the child table if it's outside the bounds of the top parent. 
    ...
    SELECT tablename INTO v_exists FROM sys_catalog.sys_tables WHERE schemaname = v_parent_schema::name AND tablename = v_partition_name::name;
 ...
 */
CREATE OR REPLACE FUNCTION create_partition_time(p_parent_table text, p_partition_times timestamp[], p_analyze boolean DEFAULT true, p_debug boolean DEFAULT false) 
RETURNS boolean
    LANGUAGE plsql SECURITY DEFINER
    AS $$
DECLARE
...
SELECT proname INTO v_function_name FROM sys_catalog.sys_proc p JOIN sys_catalog.sys_namespace n ON p.pronamespace = n.oid WHERE n.nspname = v_parent_schema AND proname = v_function_name;
