...
If you have an existing partition set and you'd like to migrate it to sys_partman, please see the migration.md file in the doc folder. This is for non-native partitioning only at this time. I'm working on a migration plan for getting non-native partition sets moved to native partition sets. If it all works out, will be included in a future version of sys_partman.
...
IMPORTANT NOTE: The template table feature in use for PostgreSQL 10+ to handle certain features is only a temporary solution to help speed up native partitioning adoption. As things are handled better natively (as they were in PG11), the use of the template table will be phased out quickly from sys_partman. So please plan ahead for quick major version upgrades if you use this feature.
...
The list of time intervals given for create_parent() below are optimized to work as fast as possible with non-native, trigger-based partitioning. Intervals other than those values are possible, but performance will take a non-trivial hit to allow such flexibility.  For native partitioning, unlike sys_partman's trigger based method, there's no differing method of partitioning for any given intervals. All possible intervals that use the native method have the same performance characteristics and are better than any trigger-based method. It is HIGHLY recommended to upgrade to PG10 if you need a partitioning interval different than the optimized ones that sys_partman provides.
...
Table inheritance in PostgreSQL does not allow a primary key or unique index/constraint on the parent to apply to all child tables. The constraint is applied to each individual table, but not on the entire partition set as a whole. For example, this means a careless application can cause a primary key value to be duplicated in a partition set. In the mean time, a python script is included with `sys_partman` that can provide monitoring to help ensure the lack of this feature doesn't cause long term harm. See **`check_unique_constraint.py`** in the **Scripts** section.

IMPORTANT NOTE: INSERT ... ON CONFLICT SUPPORT IN PG10 AND OLDER WILL BE REMOVED FROM SYS_PARTMAN IN THE NEAR FUTURE. If you require this feature, it is highly recommended you plan on upgrading to PostgreSQL 11 for proper update and upsert support.

For non-native partitioning and PG10 native partitioning, INSERT ... ON CONFLICT (upsert) is supported in the partitioning trigger as well as native partitioning, but is very limited. The major limitations are that the constraint violations that would trigger the ON CONFLICT clause only occur on individual child tables that actually contain data due to reasons explained above. Of a larger concern than data duplication is an ON CONFLICT DO UPDATE clause which may not fire and cause wildly inconsistent data if not accounted for. For situations where only new data is being inserted, upsert can provide significant performance improvements. However, if you're relying on data in older partitions to cause a constraint violation that upsert would normally handle, it likely will not work. Also, if the resulting UPDATE would end up violating the partitioning constraint of that chld table, it will fail. Neither sys_partman & PG10 native partitioning currently support UPDATES that would require moving a row from one child table to another. This is only supported in PG11+.
 ...
 * p_type - one of the following values to set the partitioning type that will be used:
    ...
    + **partman**
        - Create a trigger-based partition set using sys_partman's method of partitioning.
 ...
 * `p_interval` - the time or integer range interval for each partition. No matter the partitioning type, value must be given as text. The generic intervals of "yearly -> quarter-hour" are for time partitioning and giving one of these explicit values when using sys_partman's trigger-based partitioning will allow significantly better performance than using an arbitrary time interval. For native partitioning, any interval value is valid and will have the same performance which is always better than trigger-based.
 ...
 * `p_top_parent` - This parameter is the parent table of an already existing partition set. It tells `sys_partman` to turn all child tables of the given partition set into their own parent tables of their own partition sets using the rest of the parameters for this function. 
 * `p_native_check` - Turning an existing native partition set into a sub-partitioned set is a **destructive** process. A table must be declared natively partitioned at creation time and cannot be altered later. Therefore existing child tables must be dropped and recreated as partitioned parent tables. This flag is here to help ensure this function is not run without prior knowledge that all data in the partition set will be destroyed as part of the creation process. It must be set to "yes" to proceed with sub-partitioning a native partition set. This option can be ignored if you are created a trigger-based sys_partman partition set.
 * All other parameters to this function have the same exact purpose as those of `create_parent()`, but instead are used to tell `sys_partman` how each child table shall itself be partitioned.
 ...
 * For sub-partitioned sets, you must start partitioning data at the highest level and work your way down each level. This means you must first run this function before running create_sub_parent() to create the additional partitioning levels. Then continue running this function again on each new sub-parent once they're created. See the  sys_partman_howto.md document for a full example. IMPORTANT NOTE: this may not work as expected for native partitioning since subpartitioning a native set in sys_partman is a destructive operation. See create_sub_parent().
 ...
 * `p_jobmon` - an optional paramter to control whether `run_maintenance()` itself uses the `sys_jobmon` extension to log what it does. Whether the maintenance of a particular table uses `sys_jobmon` is controlled by the setting in the **part_config** table and this setting will have no affect on that. Defaults to true if not set.
 ...
 * List all child tables of a given partition set managed by sys_partman. Each child table returned as a single row.
 ...
 * Given a parent table managed by sys_partman (p_parent_table) and an appropriate value (time or id but given in text form for p_value), return the name of the child partition that that value would exist in.
 ...
 * Run this function to monitor that the parent tables of the partition sets that `sys_partman` manages do not get rows inserted to them.
 ...
 * If the `sys_partman` constraints already exists on the child table, the function will cleanly skip over the ones that exist and not create duplicates.
 ...
 * Drop constraints that have been created by `sys_partman` for the columns that are configured in *part_config*. This makes it easy to clean up constraints if old data needs to be edited and the constraints aren't allowing it.
 ...
 * Procedures for PG11+ to reapply the extra constraint managed by sys_partman (see Constraint Exclusion section in About section above).
 ...
 * `p_parent_table` - Parent table of an already created partition set.
 * `p_drop_constraints` - Drop all constraints managed by sys_partman. Drops constraints on all child tables including current & future.
 ...
 * `p_parent_table` - parent table of the partition set. Must be schema qualified and match a parent table name already configured in `sys_partman`.
 ...
 * This function can be used on any table inheritance set, not just ones managed by `sys_partman`.
 * The p_job_id parameter is optional. It's for internal use and allows job logging to be consolidated into the original job that called this function if applicable.
 * The p_debug parameter will show you the constraint creation statement that was used.
 ...
 * Undo a partition set created by `sys_partman`. This function MOVES the data from the child tables to either the parent table (non-native) or the given target table (native).
 ...
 * Once all child tables have been uninherited/dropped, the configuration data is removed from `sys_partman` automatically.
...
The rest are managed by the extension itself and should not be changed unless absolutely necessary.
 ...
 - `inherit_fk`
    - Set whether `sys_partman` manages inheriting foreign keys from the parent table to all children.
 ...
 - `jobmon`
    - Boolean value to determine whether the `sys_jobmon` extension is used to log/monitor partition maintenance. Defaults to true.
 ...
 * Stores all configuration data for sub-partitioned sets managed by `sys_partman`.
 ...
 * `--version`:              Print out the minimum version of `sys_partman` this script is meant to work with. The version of `sys_partman` installed may be greater than this.
 ...
 * Can also work on any non-native parent/child partition set not managed by `sys_partman` if --type option is not set.
 ...
 * A python script to dump out tables contained in the given schema. Uses sys_dump, creates a SHA-512 hash file of the dump file, and then drops the table.
 ...
 * Tables are not dropped if sys_dump does not return successfully.
 * The connection options for psycopg and sys_dump were separated out due to distinct differences in their requirements depending on your database connection configuration. 
 * All dump_* option defaults are the same as they would be for sys_dump if they are not given.
 * Will work on any given schema, not just the one used to manage `sys_partman` retention.
 ...
 * `--connection (-c)`:      Connection string for use by psycopg. 
                             Role used must be able to select from sys_catalog.sys_tables in the relevant database and drop all tables in the given schema. 
                             Defaults to "host=" (local socket). Note this is distinct from the parameters sent to sys_dump. 
 * `--output (-o)`:          Path to dump file output location. Default is where the script is run from.
 * `--dump_database (-d)`:   Used for sys_dump, same as its --dbname option or final database name parameter.
 * `--dump_host`:            Used for sys_dump, same as its --host option.
 * `--dump_username`:        Used for sys_dump, same as its --username option.
 * `--dump_port`:            Used for sys_dump, same as its --port option.
 * `--sys_dump_path`:         Path to sys_dump binary location. Must set if not in current PATH.
 * `--Fp`:                   Dump using sys_dump plain text format. Default is binary custom (-Fc).
 ...
 * `--nonpartman`            If the partition set you are running this on is not managed by sys_partman, set this flag otherwise this script may not work. 
                             Note that the sys_partman extension is still required to be installed for this to work since it uses certain internal functions. 
 ...
 * `--parent (-p)`:           Parent table of an already created partition set. (Required)
 * `--connection (-c)`:       Connection string for use by psycopg. Defaults to "host=" (local socket).
 * `--drop_constraints (-d)`: Drop all constraints managed by `sys_partman`. Drops constraints on ALL child tables in the partition set.
 ...
 * `--quiet (-q)`:            Switch setting to stop all output during and after partitioning undo.
 * `--dryrun`:                Show what the script will do without actually running it against the database. Highly recommend reviewing this before running.
 * `--nonpartman`             If the partition set you are running this on is not managed by sys_partman, set this flag. Otherwise internal sys_partman functions are used and this script may not work. 
